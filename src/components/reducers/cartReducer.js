...
import { INC_QUANTITY_ITEM_ADMIN,DEC_QUANTITY_ITEM_ADMIN,INC_PRICE_ITEM_ADMIN,DEC_PRICE_ITEM_ADMIN,RESET,SET_TIMER } from '../actions/action-types/admin-actions'

const initStateOfCart = {
    items: [
        { id: 1, title: 'Su', desc: "Erikli", price: 25, sayi:5,img: Item1 },
        { id: 2, title: 'Soda', desc: "Beypazarı", price: 35, sayi:5,img: Item2 },
        { id: 3, title: 'Kola', desc: "Pepsi", price: 45, sayi:5,img: Item3 },
    ],
    consuptions: [
        { id: 1, title: 'Lightining', desc: "", price: 2, sayi:0 },
        { id: 2, title: 'Robotics', desc: "", price: 2, sayi:0 },
        { id: 3, title: 'Others', desc: "", price: 2, sayi:0 },
    ],
    addedItems: [],
    total: 0,
...
const cartReducer = (state = initStateOfCart, action) => {
    ...
    if (action.type === CHECK_OUT) {     
        if (state.consumerCashTotal>=state.total) {    
            state.consuptions(1).quantity =state.addedItems(0).quantity +state.addedItems(1).quantity  + state.addedItems(2).quantity ;
    ...
    if (action.type === REFUND) {    
        ...
        state.addedItems= [];
        state.total =0; 
        state.deadline = Date.now()+ 5*1000*60  
